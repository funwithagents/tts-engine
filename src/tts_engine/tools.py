...
(e.g. `TTSTools(engine).speak`) directly, and library code can call them for
the guarded, non-raising contract instead of raw `engine.speak`.
"""
...
class TTSTools:
    ...
    bound method (`TTSTools(engine).speak`) is directly usable as a tool — its
