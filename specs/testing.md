...
code:
  ...
  - tests-e2e/fixtures/hello.wav
  - tests-e2e/fixtures/default.wav
tests:
---
...
| Live / e2e | `tests-e2e/` | real provider backends + audio hw | no | **no** |
...
- **Module-agnostic — `test_mcp.py`** (default module). The MCP transport doesn't change with the backend, so it runs once: it starts a `tts-engine-mcp` subprocess (temp config built from `default_module()`) and calls the `say` tool over StreamableHTTP with a real MCP client. Uses the `server_url` fixture. The MCP stack (`mcp`, `uvicorn`) is an extra for consumers but part of the `dev` group, so this test needs no skip gate of its own (see [project.md](project.md), "Dependency strategy for transports").

**Default module vs per-module configs.** `support.default_module()` is the single place the backend the module-agnostic MCP test drives is chosen; the `MODULES` table holds the *per-module configs* for the parametrized conformance tests. The default module is the **`tone` fixture module** ([tone-module.md](modules/tone.md)): it needs no key, no extra, and no files, so the transport test runs on every machine with audio hardware instead of skipping. It is a test default only — it says nothing about which *provider* a user should pick, and the `MODULES` table treats every provider symmetrically.

**`MODULES` rows.** One per registered module: `elevenlabs` (gated on `ELEVENLABS_API_KEY` and the `elevenlabs` extra), `pocket` (gated on the `pocket` extra), `gradium` (gated on `GRADIUM_API_KEY` and the `gradium` extra), `tone` (never skips), and `audiofile` (never skips; its config points `base_dir` at `tests-e2e/fixtures/`, which holds two small committed 16-bit mono WAV files, `hello.wav` and `default.wav`).

The `tests/` tier mirrors the `src/tts_engine/` module layout (`test_<module>.py` — e.g. `test_engine.py`, `test_tools.py`, `test_mcp.py`, `test_config.py` — one `modules/test_<backend>.py` per TTS module — `tests/modules/` stays flat even though each backend is a package under `src/tts_engine/modules/` — plus the `test_project_map.py` drift-guard); `tests-e2e/` is organized around these live scenarios rather than modules.
...
- **No secrets in the tree.** The hardcoded module configs carry `api_key_env: "ELEVENLABS_API_KEY"` — the *name* of the env var holding the key, never the key itself (see [elevenlabs-module.md](modules/elevenlabs.md), "API key resolution"). The live tier is turned on by exporting `ELEVENLABS_API_KEY`; the MCP subprocess inherits the parent environment, so the same variable reaches the server it spawns. This is what makes the tier CI-ready.
- **`support.require_module(module_type, config)` is the skip gate**, and both entry points funnel through it — `default_module()` calls it before returning, and the parametrized `test_modules.py` calls it per row. Backends gate differently, so it generalizes the key check: it skips when either (a) the config's `api_key_env` names an unset variable (an API backend without its key, like elevenlabs), **or** (b) the backend's library isn't importable (a provider whose packaging extra isn't installed: `elevenlabs` needing `elevenlabs`, `pocket` needing `pocket_tts`, `gradium` needing `gradium` — see [project.md](project.md), "Dependency strategy for TTS backends"). A module that needs no key and no extra (the fixture modules) never skips. The module type → gating-library map (`_REQUIRED_IMPORT`) lives beside the `MODULES` table and has one row per provider.
