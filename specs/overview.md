...
`tts-engine` is a streaming text-to-speech engine. Its core is a reusable `TTSEngine` that synthesizes a text string through a pluggable TTS backend — a cloud provider (e.g. ElevenLabs) or a local in-process model (e.g. pocket-tts), each installed as an optional extra with no default — and plays the audio in real-time on the machine it runs on. Two built-in fixture modules (`tone`, `audiofile`) stand in for a provider in tests and demos. The engine is usable two ways:
