...
The `dev` dependency group additionally lists `elevenlabs` and `miniaudio` (light, pure-Python) so the ElevenLabs unit tests run unchanged under `uv sync --dev`, and `mcp<2` and `uvicorn` so the MCP unit tests and the live MCP test run unchanged too; `pocket`'s `torch` stays out of `dev` and its unit tests fake the library.
