...
- [x] Implement `AudioPlayer.__init__(device=None)`: store device, do not open stream yet
- [x] Implement `AudioPlayer.feed(chunk: bytes) -> None`:
...
- [x] Implement `AudioPlayer.drain() -> None`:
  - If stream is open: call `stream.stop()` then `stream.close()`
  - Reset internal stream reference to `None`

### Tests (`tests/test_audio.py`)

- [x] `feed()` opens the stream on first call and does not open it again on subsequent calls
- [x] `drain()` closes the stream; calling `drain()` twice does not raise
- [x] `drain()` before any `feed()` does not raise
- [x] `feed()` with empty bytes does not raise and does not open the stream
